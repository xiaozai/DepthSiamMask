...
class SiamMask(nn.Module):
    ...
    def rgbd_feature_fusion(self, f_rgb, f_d):
        return self.fusion_model(f_rgb, f_d)
    ...
    def run(self, template, template_d, search, search_d, softmax=False):
        """
        run network
        """
        template_feature = self.feature_extractor(template)
        template_feature_d = self.feature_extractor(template_d)
        search_feature = self.feature_extractor(search)
        search_feature_d = self.feature_extractor(search_d)

        ''' RGBD feature fusion '''
        template_feature = self.rgbd_feature_fusion(template, template_d)
        search_feature = self.rgbd_feature_fusion(search_feature, search_feature_d)
    ...
    def forward(self, input):
        ...
        template_d = input['template_d']
        search = input['search']
        search_d = input['search_d']
        ...
        rpn_pred_cls, rpn_pred_loc, rpn_pred_mask, template_feature, search_feature = \
            self.run(template, template_d, search, search_d, softmax=self.training)
    ...
    def template(self, z, z_d):
        zf = self.feature_extractor(z)
        zf_d = self.feature_extractor(z_d)
    ...
    def track(self, x, x_d, cls_kernel=None, loc_kernel=None, softmax=False):
        xf = self.feature_extractor(x)
        xf_d = self.feature_extractor(x_d)
        xf = self.rgbd_feature_fusion(x, x_d)
